        ...
        },
        {
            ...
            "category_products": [
                ...
                },
                {
                    ...
                    "second_image": "https://assets.myntassets.com/h_720,q_90,w_540/v1/assets/images/12834502/2020/11/26/32c3c29c-5be7-4978-8e03-93b981a4993f1606382198167WomenBlackMultiColourPrintedKurti2.jpg"
                },
                {
                    "id": "4w",
                    "title": "Women Pant Set",
                    "price": "2199",
                    "compare_at_price": "2499",
                    "vendor": "Metro-Fashion",
...
function displayProducts(categories = [], searchTerm = '') {
    const productsContainer = document.getElementById('products');
    productsContainer.innerHTML = '';

    categories.forEach(categoryName => {
        const category = productsData.categories.find(cat => cat.category_name === categoryName);
        if (category) {
            category.category_products
                ...
                .forEach(product => {
                    ...
                    productDiv.innerHTML = `
                        ...
                        <p class="product-price">₹${product.price} <span class="compare-at-price">₹${product.compare_at_price}</span></p>
...
function filterProducts(categoryName) {
    ...
    displayProducts([categoryName], searchTerm);
}

document.getElementById('search').addEventListener('input', function() {
    const categoryButtons = document.querySelectorAll('.buttons button');
    const activeCategory = Array.from(categoryButtons).find(button => button.classList.contains('active'));
    if (activeCategory) {
        filterProducts(activeCategory.textContent);
    } else {
        displayProducts(['Men', 'Women', 'Kids'], document.getElementById('search').value);
    }
});

// Display all categories by default
displayProducts(['Men', 'Women', 'Kids']);
